...
exports.publish = function(data, opts, tutorials) {
    ...
    for (var d in docs) {
        ...
        if (doc.kind === 'member') {
            ...
            output[parent][doc.name] = {};
            
            // TODO handle multiple types
            var firstType = doc.properties[0].type.names[0];
            if (output['!define'][firstType] !== undefined) {
                var typedef = output['!define'][firstType];
                output[parent][doc.name].type = typedef.type.names[0];
                output[parent][doc.name].description = typedef.description;
            } else {
                output[parent][doc.name].type = firstType;
                output[parent][doc.name].description = doc.properties[0].description;
            }
        }
    }
