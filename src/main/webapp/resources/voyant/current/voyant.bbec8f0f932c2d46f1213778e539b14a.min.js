...
parentPanel:this}).load({params:Ext.apply(this.getApiParams(),{query:a.term,start:a.start,limit:b}),callback:function(d,e,f){f&&(this.updateDataForNode(a.id,{start:a.start+b}),this.loadFromCorpusCollocateRecords(d,a.id))},scope:this})}});Ext.define("Voyant.panel.Constellations",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.constellations",statics:{i18n:{title:"Constellations",options:"Options",termSearch:"Term Search",cutoff:"Similarity Threshold",numTerms:"Terms",analysis:"Analysis",ca:"Correspondence Analysis",pca:"Principal Component Analysis",tsne:"t-SNE",distance:"Distance",cosine:"Cosine Similarity",euclidean:"Euclidean",help:"Constellations produces a specialized network graph of high frequency terms that are connected by similar word vectors.",
helpTip:"Constellations produces a specialized network graph of high frequency terms that are connected by similar word vectors."},api:{analysis:"ca",docId:void 0,limit:50,dimensions:3,stopList:"auto"},glyph:"xf1e0@FontAwesome"},config:{caStore:void 0,pcaStore:void 0,tsneStore:void 0,allNodeData:void 0,allEdgeData:void 0,simCutoff:25,simMin:0,simMax:1,allSims:[],simWin:void 0,distanceType:"cosine",vis:void 0,simulation:void 0,physics:{nodeGravity:-500,springLength:50},zoom:void 0,zoomExtent:[.25,
8],termSearchTimeout:null,options:[{xtype:"stoplistoption"}]},constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){this.setCaStore(Ext.create("Voyant.data.store.CAAnalysis",{parentPanel:this,listeners:{load:this.handleData,scope:this}}));this.setPcaStore(Ext.create("Voyant.data.store.PCAAnalysis",{parentPanel:this,listeners:{load:this.handleData,scope:this}}));this.setTsneStore(Ext.create("Voyant.data.store.TSNEAnalysis",
{parentPanel:this,listeners:{load:this.handleData,scope:this}}));Ext.apply(this,{title:this.localize("title"),layout:"border",items:[{xtype:"container",region:"center",layout:"fit",style:"backgroundColor: #fff",items:{xtype:"component",itemId:"visParent",listeners:{resize:function(a,b,c){if(a=a.getEl().down("svg"))d3.select(a.dom).attr("width",b).attr("height",c),Ext.Function.defer(this.zoomToFit,50,this)},scope:this}}},{title:this.localize("options"),xtype:"panel",width:200,region:"west",split:!0,
...
initGraph:function(a,b){var c=this.down("#visParent").getEl();c.update("");var d=c.getWidth(),e=c.getHeight(),f=d3.select(c.dom).append("svg").attr("width",d).attr("height",e),g=f.append("g");g.append("g").attr("class","edges");g.append("g").attr("class","nodes");g.append("g").attr("class","labels");this.setVis(g);c=d3.zoom().scaleExtent(this.getZoomExtent()).on("zoom",function(h){g.attr("transform",h.transform)});this.setZoom(c);f.call(c);c=this.getPhysics();this.setSimulation(d3.forceSimulation(a).force("x",
d3.forceX(d/2)).force("y",d3.forceY(e/2)).force("link",d3.forceLink(b).id(function(h){return h.id}).distance(c.springLength)).force("charge",d3.forceManyBody().strength(c.nodeGravity)).force("center",d3.forceCenter(d/2,e/2)).on("tick",function(){f.select(".edges").selectAll("line").attr("x1",function(h){return h.source.x}).attr("y1",function(h){return h.source.y}).attr("x2",function(h){return h.target.x}).attr("y2",function(h){return h.target.y});f.select(".nodes").selectAll("circle").attr("cx",function(h){return h.x}).attr("cy",
function(h){return h.y});f.select(".labels").selectAll("text").attr("x",function(h){return h.x+15}).attr("y",function(h){return h.y+5});.075>this.getSimulation().alpha()&&this.stopSimulation()}.bind(this)).on("end",function(){Ext.Function.defer(this.zoomToFit,100,this)}.bind(this)));this.getSimulation().stop()},updateGraph:function(){this.stopSimulation();var a=this.down("#termSearch").getValue(),b=[];null===this.getNodeById(a,this.getAllNodeData())&&(a.includes("-")||a.includes("+"))&&b.push(this.parseSelection(a,
this.getAllNodeData()));console.log(a,b[0]);a=$jscomp.makeIterator(this.doFilter(this.getAllNodeData(),this.getAllEdgeData(),a));nodes=a.next().value;edges=a.next().value;this.getVis().select(".labels").selectAll("text").data(nodes).join("text").classed("hidden",!1).classed("label",!0).style("cursor","pointer").attr("font-size","16px").attr("font-family",'"Palatino Linotype", "Book Antiqua", Palatino, serif').text(function(c){return c.id}).on("click",this.handleNodeClick.bind(this)).call(d3.drag().on("start",
this.handleDragStart.bind(this)).on("drag",this.handleDrag.bind(this)).on("end",this.handleDragEnd.bind(this)));this.getVis().select(".nodes").selectAll("circle").data(nodes,function(c){return c.id}).join("circle").classed("node",!0).classed("selected",function(c){return!0===c.selected}).style("cursor","pointer").style("stroke",function(c){return c.selected?"#ff8800":"#6baed6"}).style("stroke-width",1).style("fill",function(c){return c.selected?"#ffcd93ff":"#c6dbef"}).attr("r",function(c){return!0===
c.selected?20:10}).on("click",this.handleNodeClick.bind(this)).call(d3.drag().on("start",this.handleDragStart.bind(this)).on("drag",this.handleDrag.bind(this)).on("end",this.handleDragEnd.bind(this)));this.getVis().select(".edges").selectAll("line").data(edges).join("line").classed("edge",!0).style("stroke","#000").style("stroke-opacity",.1);this.getSimulation().nodes(nodes).force("link").links(edges);this.getSimulation().alpha(1).restart()},stopSimulation:function(){this.getSimulation()&&this.getSimulation().alpha(-1)},
handleNodeClick:function(a,b){a.stopImmediatePropagation();a.preventDefault();this.dispatchEvent("termsClicked",this,[b.id])},handleDragStart:function(a){this.getSimulation().alpha(.3).restart();a.subject.fx=a.subject.x;a.subject.fy=a.subject.y},handleDrag:function(a){this.getSimulation().alpha(.3);a.subject.fx=a.x;a.subject.fy=a.y},handleDragEnd:function(a){a.subject.fx=null;a.subject.fy=null},loadFromApis:function(){this.stopSimulation();var a={};Ext.apply(a,this.getApiParams());delete a.analysis;
"tsne"===this.getApiParam("analysis")?this.getTsneStore().load({params:a}):"pca"===this.getApiParam("analysis")?this.getPcaStore().load({params:a}):this.getCaStore().load({params:a})},handleData:function(a){var b=a.getAt(0).getTokens().map(function(h){return h.getData()});"ca"===this.getApiParam("analysis")&&(b=b.filter(function(h){return"term"===h.category}));a=b.map(function(h){return{id:h.term}});var c=[],d=this.getDistanceType(),e=[];b=$jscomp.makeIterator(this.combinations(b));for(var f=b.next();!f.done;f=
b.next()){f=f.value;var g=this.distance(f[0].vector,f[1].vector,d);c.push({source:f[0].term,target:f[1].term,sim:g});e.push(g)}this.setAllSims(e.sort(function(h,k){return h-k}));this.setSimMin(this.arrayMin(e));this.setSimMax(this.arrayMax(e));console.log(this.getSimMin(),this.getSimMax());this.setAllNodeData(a);this.setAllEdgeData(c);this.initGraph(a,c);this.updateGraph();this.down("#simGraph").setValues(this.getAllSims())},doFilter:function(a,b,c){var d=this.getSimCutoff()/100,e=this.easeIn(d),
f=this.map(e,0,1,this.getSimMin(),this.getSimMax());console.log(d,e,f);d={};d["0:"+f]="#ff8800";this.down("#simGraph").setValueSpots(d);if(void 0===a||void 0===b)return[[],[]];var g=new Set;b=b.filter(function(h){if(h.sim<=f)return g.add(h.source.id),g.add(h.target.id),!0});a=a.filter(function(h){h.selected=!1;return h.id===c?h.selected=!0:g.has(h.id)});0===a.length&&console.warn("no nodes!");console.log("num edges",b.length);return[a,b]},distance:function(a,b,c){if("euclidean"===c)return Math.sqrt(a.reduce(function(e,
f,g){return e+Math.pow(f-b[g],2)},0));c=a.map(function(e,f){return e*b[f]}).reduce(function(e,f){return e+f},0);a=Math.sqrt(a.map(function(e){return e*e}).reduce(function(e,f){return e+f},0));var d=Math.sqrt(b.map(function(e){return e*e}).reduce(function(e,f){return e+f},0));return c/(a*d)*-1+1},combinations:function $jscomp$generator$function$873951774$65(a){var c,d,e,f,g,h,k,l;return $jscomp.generator.createGenerator($jscomp$generator$function$873951774$65,function(m){switch(m.nextAddress){case 1:c=
$jscomp.makeIterator(a.entries()),d=c.next();case 2:if(d.done){m.jumpTo(0);break}e=d.value;f=e[0];g=e[1];h=$jscomp.makeIterator(a.slice(f+1));k=h.next();case 5:if(k.done){d=c.next();m.jumpTo(2);break}l=k.value;return m.yield([g,l],6);case 6:k=h.next(),m.jumpTo(5)}})},getNodeById:function(a,b){b=$jscomp.makeIterator(b);for(var c=b.next();!c.done;c=b.next())if(c=c.value,c.id===a)return c;return null},parseSelection:function(a,b){var c=a.split(" ");a=null;var d=this.addVectors;c=$jscomp.makeIterator(c);
for(var e=c.next();!e.done;e=c.next()){e=e.value;var f=this.getNodeById(e,b);"+"===e?d=this.addVectors:"-"===e?d=this.subtractVectors:null!==f?a=d(a,[f.vx,f.vy]):console.log("unknown word",e)}return a},addVectors:function(a,b){return null===a?b:null===b?a:a.map(function(c,d){return c+b[d]})},subtractVectors:function(a,b){return null===a?b:null===b?a:a.map(function(c,d){return c-b[d]})},arrayMin:function(a){for(var b=a.length,c=Infinity;b--;)a[b]<c&&(c=a[b]);return c},arrayMax:function(a){for(var b=
a.length,c=-Infinity;b--;)a[b]>c&&(c=a[b]);return c},easeIn:function(a){return a*a*a},map:function(a,b,c,d,e){return d+(a-b)/(c-b)*(e-d)},zoomToFit:function(a,b){var c=this.getVis().node().getBBox(),d=c.width,e=c.height,f=c.x+d/2,g=c.y+e/2;c=this.getVis().node().parentElement;var h=c.getBoundingClientRect(),k=h.width;h=h.height;a=(a||.8)/Math.max(d/k,e/h);f=[k/2-a*f,h/2-a*g];1>d||d3.select(c).transition().duration(b||500).call(this.getZoom().transform,d3.zoomIdentity.translate(f[0],f[1]).scale(a))}});Ext.define("Voyant.panel.Contexts",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],requires:["Voyant.data.store.Contexts"],alias:"widget.contexts",isConsumptive:!0,statics:{i18n:{},api:{query:void 0,docId:void 0,docIndex:void 0,stopList:"auto",context:5,expand:50,columns:void 0,sort:void 0,dir:void 0,termColors:"categories"},glyph:"xf0ce@FontAwesome"},config:{options:[{xtype:"stoplistoption"},{xtype:"categoriesoption"},{xtype:"termcolorsoption"}]},constructor:function(){this.mixins["Voyant.util.Api"].constructor.apply(this,
...
//# sourceMappingURL=voyant.bbec8f0f932c2d46f1213778e539b14a.min.js.map