...
arguments)},initComponent:function(){this.setCaStore(Ext.create("Voyant.data.store.CAAnalysis",{parentPanel:this,listeners:{load:this.handleData,scope:this}}));this.setPcaStore(Ext.create("Voyant.data.store.PCAAnalysis",{parentPanel:this,listeners:{load:this.handleData,scope:this}}));this.setTsneStore(Ext.create("Voyant.data.store.TSNEAnalysis",{parentPanel:this,listeners:{load:this.handleData,scope:this}}));Ext.apply(this,{title:this.localize("title"),layout:"border",items:[{xtype:"container",region:"center",
layout:"fit",style:"backgroundColor: #fff",items:{xtype:"component",itemId:"visParent"}},{title:this.localize("options"),xtype:"panel",width:200,region:"west",split:!0,collapsible:!0,scrollable:"y",layout:{type:"vbox",align:"stretch"},defaults:{xtype:"button",margin:"5",labelAlign:"top"},items:[{xtype:"documentselectorbutton"},{text:this.localize("analysis"),itemId:"analysis",glyph:"xf1ec@FontAwesome",menu:{items:[{text:this.localize("ca"),itemId:"analysis_ca",group:"analysis",xtype:"menucheckitem"},
{text:this.localize("pca"),itemId:"analysis_pca",group:"analysis",xtype:"menucheckitem"},{text:this.localize("tsne"),itemId:"analysis_tsne",group:"analysis",xtype:"menucheckitem"}],listeners:{render:function(a){a.child("#analysis_"+this.getApiParam("analysis")).setChecked(!0)},click:function(a,b){void 0!==b&&(a=b.getItemId().split("_")[1],a!==this.getApiParam("analysis")&&(this.setApiParam("analysis",a),this.loadFromApis()))},scope:this}}},{xtype:"slider",fieldLabel:this.localize("numTerms"),minValue:10,
maxValue:250,increment:5,listeners:{render:function(a){a.setValue(this.getApiParam("limit"))},changecomplete:function(a,b){this.setApiParam("limit",b);this.loadFromApis()},scope:this}},{xtype:"textfield",fieldLabel:this.localize("termSearch"),itemId:"termSearch",listeners:{change:function(a,b){null!==this.getTermSearchTimeout()&&clearTimeout(this.getTermSearchTimeout());this.setTermSearchTimeout(setTimeout(this.updateGraph.bind(this),500))},scope:this}},{xtype:"slider",fieldLabel:this.localize("cutoff"),
minValue:1,maxValue:100,increment:1,listeners:{render:function(a){a.setValue(this.getSimCutoff())},changecomplete:function(a,b){this.setSimCutoff(b);this.updateGraph()},scope:this}},{xtype:"container",layout:"fit",items:{itemId:"simGraph",xtype:"sparklineline",minSpotColor:"",maxSpotColor:"",spotColor:"",disableTooltips:!0,width:200,height:50}}]}]});this.on("boxready",function(a,b){},this);this.on("loadedCorpus",function(a,b){this.isVisible()&&(this.getCaStore().setCorpus(b),this.getPcaStore().setCorpus(b),
this.getTsneStore().setCorpus(b),this.loadFromApis())},this);this.on("corpusSelected",function(a,b){this.setApiParam("docId",void 0);this.loadFromApis()},this);this.on("documentsSelected",function(a,b){this.setApiParam("docId",b);this.loadFromApis()},this);this.on("activate",function(){this.getCorpus()},this);this.on("query",function(a,b){this.loadFromQuery(b)},this);this.callParent(arguments)},initGraph:function(a,b){var c=this.down("#visParent").getEl();c.update("");var d=c.getWidth(),e=c.getHeight(),
f=d3.select(c.dom).append("svg").attr("width",d).attr("height",e),g=f.append("g");g.append("g").attr("class","edges");g.append("g").attr("class","nodes");g.append("g").attr("class","labels");this.setVis(g);c=d3.zoom().scaleExtent(this.getZoomExtent()).on("zoom",function(h){g.attr("transform",h.transform)});this.setZoom(c);f.call(c);c=this.getPhysics();this.setSimulation(d3.forceSimulation(a).force("x",d3.forceX(d/2)).force("y",d3.forceY(e/2)).force("link",d3.forceLink(b).id(function(h){return h.id}).distance(c.springLength)).force("charge",
d3.forceManyBody().strength(c.nodeGravity)).force("center",d3.forceCenter(d/2,e/2)).on("tick",function(){f.select(".edges").selectAll("line").attr("x1",function(h){return h.source.x}).attr("y1",function(h){return h.source.y}).attr("x2",function(h){return h.target.x}).attr("y2",function(h){return h.target.y});f.select(".nodes").selectAll("circle").attr("cx",function(h){return h.x}).attr("cy",function(h){return h.y});f.select(".labels").selectAll("text").attr("x",function(h){return h.x+15}).attr("y",
function(h){return h.y+5});.075>this.getSimulation().alpha()&&this.stopSimulation()}.bind(this)).on("end",function(){Ext.Function.defer(this.zoomToFit,100,this)}.bind(this)));this.getSimulation().stop()},updateGraph:function(){this.stopSimulation();var a=this.down("#termSearch").getValue();console.log(a);a=$jscomp.makeIterator(this.doFilter(this.getAllNodeData(),this.getAllEdgeData(),a));nodes=a.next().value;edges=a.next().value;this.getVis().select(".labels").selectAll("text").data(nodes).join("text").classed("hidden",
!1).classed("label",!0).style("cursor","pointer").attr("font-size","16px").attr("font-family",'"Palatino Linotype", "Book Antiqua", Palatino, serif').text(function(b){return b.id}).on("click",this.handleNodeClick.bind(this)).call(d3.drag().on("start",this.handleDragStart.bind(this)).on("drag",this.handleDrag.bind(this)).on("end",this.handleDragEnd.bind(this)));this.getVis().select(".nodes").selectAll("circle").data(nodes,function(b){return b.id}).join("circle").classed("node",!0).classed("selected",
...
//# sourceMappingURL=voyant.8cdebb4fe6e74d2f9284308d2d02863b.min.js.map