...
a.getAt(0).getTokens().map(function(h){return h.getData()});"ca"===this.getApiParam("analysis")&&(b=b.filter(function(h){return"term"===h.category}));a=b.map(function(h){return{id:h.term}});var c=[],d=this.getDistanceType(),e=[];b=$jscomp.makeIterator(this.combinations(b));for(var f=b.next();!f.done;f=b.next()){f=f.value;var g=this.distance(f[0].vector,f[1].vector,d);c.push({source:f[0].term,target:f[1].term,sim:g});e.push(g)}this.setAllSims(e.sort(function(h,k){return h-k}));this.setSimMin(this.arrayMin(e));
this.setSimMax(this.arrayMax(e));console.log(this.getSimMin(),this.getSimMax());this.setAllNodeData(a);this.setAllEdgeData(c);this.initGraph(a,c);this.updateGraph();this.down("#simGraph").setValues(this.getAllSims())},doFilter:function(a,b,c){var d=this.getSimCutoff()/100,e=this.easeIn(d),f=this.map(e,0,1,this.getSimMin(),this.getSimMax());console.log(d,e,f);d={};d["0:"+f]="#ff8800";this.down("#simGraph").setValueSpots(d);if(void 0===a||void 0===b)return[[],[]];var g=new Set;b=b.filter(function(h){if(h.sim<=
f)return g.add(h.source.id),g.add(h.target.id),!0});a=a.filter(function(h){h.selected=!1;return h.id===c?h.selected=!0:g.has(h.id)});0===a.length&&console.warn("no nodes!");console.log("num edges",b.length);return[a,b]},distance:function(a,b,c){if("euclidean"===c)return Math.sqrt(a.reduce(function(e,f,g){return e+Math.pow(f-b[g],2)},0));c=a.map(function(e,f){return e*b[f]}).reduce(function(e,f){return e+f},0);a=Math.sqrt(a.map(function(e){return e*e}).reduce(function(e,f){return e+f},0));var d=Math.sqrt(b.map(function(e){return e*
e}).reduce(function(e,f){return e+f},0));return c/(a*d)*-1+1},combinations:function $jscomp$generator$function$873951774$59(a){var c,d,e,f,g,h,k,l;return $jscomp.generator.createGenerator($jscomp$generator$function$873951774$59,function(m){switch(m.nextAddress){case 1:c=$jscomp.makeIterator(a.entries()),d=c.next();case 2:if(d.done){m.jumpTo(0);break}e=d.value;f=e[0];g=e[1];h=$jscomp.makeIterator(a.slice(f+1));k=h.next();case 5:if(k.done){d=c.next();m.jumpTo(2);break}l=k.value;return m.yield([g,l],
6);case 6:k=h.next(),m.jumpTo(5)}})},arrayMin:function(a){for(var b=a.length,c=Infinity;b--;)a[b]<c&&(c=a[b]);return c},arrayMax:function(a){for(var b=a.length,c=-Infinity;b--;)a[b]>c&&(c=a[b]);return c},easeIn:function(a){return a*a*a},map:function(a,b,c,d,e){return d+(a-b)/(c-b)*(e-d)},zoomToFit:function(a,b){var c=this.getVis().node().getBBox(),d=c.width,e=c.height,f=c.x+d/2,g=c.y+e/2;c=this.getVis().node().parentElement;var h=c.getBoundingClientRect(),k=h.width;h=h.height;a=(a||.8)/Math.max(d/
k,e/h);f=[k/2-a*f,h/2-a*g];1>d||d3.select(c).transition().duration(b||500).call(this.getZoom().transform,d3.zoomIdentity.translate(f[0],f[1]).scale(a))}});Ext.define("Voyant.panel.Contexts",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],requires:["Voyant.data.store.Contexts"],alias:"widget.contexts",isConsumptive:!0,statics:{i18n:{},api:{query:void 0,docId:void 0,docIndex:void 0,stopList:"auto",context:5,expand:50,columns:void 0,sort:void 0,dir:void 0,termColors:"categories"},glyph:"xf0ce@FontAwesome"},config:{options:[{xtype:"stoplistoption"},{xtype:"categoriesoption"},{xtype:"termcolorsoption"}]},constructor:function(){this.mixins["Voyant.util.Api"].constructor.apply(this,
...
//# sourceMappingURL=voyant.dbd8a099bff312b98b7ee7edfab62e26.min.js.map