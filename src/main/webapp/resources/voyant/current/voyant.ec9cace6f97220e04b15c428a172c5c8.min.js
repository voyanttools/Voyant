...
function(){clearTimeout(m);g.clear();this.toggleLoadingText();if(0<p.length){var r=function(H,F,I){for(var E=0;E<I.width;E+=this.COARSENESS)for(var D=0;D<I.height;D+=this.COARSENESS)I.mask[E]&&I.mask[E][D]&&(this.grid[E+H]||(this.grid[E+H]=[]),this.grid[E+H][D+F]=I)},v=function(H){H.alpha+=.25;H.draw(g.context)},x=function(H,F,I,E){r(H,F,I);I.x=H;I.y=F;I.live=!0;E&&I.draw(g.context)},z=function(H,F,I,E,D){for(var K=0;K<=E;K+=this.COARSENESS)for(var N=0;N<=I;N+=this.COARSENESS)if(D[K]&&D[K][N]&&null!=
this.grid[K+H]&&null!=this.grid[K+H][N+F])return!0;return!1},L=function(){var H=this.wordsToArrange-1,F=g.canvas.width,I=g.canvas.height,E=.5*F,D=.5*I;do{var K=p[l];if(void 0!==K){for(var N=Math.random()*Math.PI,B=.25*Math.random()*K.size,O=.5*(Math.random()-.5),M=.5*K.width,A=.5*K.height;;){var G=Math.floor((E+Math.cos(N)*B*this.ratio-M)/this.COARSENESS)*this.COARSENESS;var J=Math.floor((D+Math.sin(N)*B-A)/this.COARSENESS)*this.COARSENESS;var P=G+M>=F||J+A>=I?!0:z(G,J,K.height,K.width,K.mask);if(!P)break;
N+=O;B+=.05}x(G,J,K);if(g.useFadeEffect)for(G=K.alpha=0;G<l;G++)J=p[G],1>J.alpha&&v(J);else K.alpha=1,K.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(H--)};this.grid=[];l=0;L=L.createDelegate(this);z=z.createDelegate(this);x=x.createDelegate(this);v=v.createDelegate(this);r=r.createDelegate(this);this.doingArrange=!0;m=setInterval(L,50)}else alert("Error: There are no words to arrange.")};this.toggleLoadingText=function(r){g.context.save();
...
B.ids={};c(B.ids,A.info.ids);c(B.ids,M.info.ids);B.count=Object.keys(B.ids).length;K=Object.keys(B.ids);if(A.info.origIDs||M.info.origIDs)B.origIDs={},c(B.origIDs,A.info.origIDs),c(B.origIDs,M.info.origIDs),B.origCount=Object.keys(B.origIDs).length;A.info=B;M.info=B;G=Math.max(M.maxChildren,A.maxChildren);if(isNaN(G)||0==G)return L=!1,e;u?N=d3.scaleLog().range([8,14]):(N=function(){return 14},N.domain=function(){});N.domain([Math.min(M.minCount,A.minCount),M.info.count]);var J=g-20-20,P=h-20-20;f.forEach(function(Q,
C){C=d3.select(Q).select("svg");null==C.node()?(C=d3.select(Q).append("svg").attr("width",J+20+20).attr("height",P+20+20).attr("cursor","move").call(d3.zoom().scaleExtent([1,1]).on("zoom",function(){var R=d3.event.transform.x+J/2,S=d3.event.transform.y+P/2;d3.select(Q).select("svg \x3e g").attr("transform","translate("+R+","+S+")")})),C.append("g").attr("transform","translate("+J/2+","+P/2+")")):(C.attr("width",J+20+20).attr("height",P+20+20),C.selectAll("g *").remove());E=new doubletree.Tree(C.select("g"),
g,h,M,!0,w,H,N,n,p,q,r,v,x,z);D=new doubletree.Tree(C.select("g"),g,h,A,!1,w,H,N,n,p,q,r,v,x,z)});E.handleAltPress=m.alt;D.handleAltPress=m.alt;E.handleShiftPress=m.shift;D.handleShiftPress=m.shift;E.handleClick=m.click;D.handleClick=m.click;L=!0;return e};e.setupFromArrays=function(B,O,M,A,G,J,P,Q){void 0==G&&F&&(G=F.caseSensitive());void 0==J&&F&&(J=F.fieldNames());void 0==P&&F&&(P=F.fieldDelim());void 0==Q&&F&&(Q=F.distinguishingFieldsArray());F=new doubletree.Trie(G,J,P,Q);I=new doubletree.Trie(G,
J,P,Q);G=O.length;for(J=0;J<G;J++){P=A?A[J]:J;Q=O[J];var C=B[J].slice(),R=M[J].slice();C.push(Q);C.reverse();R.unshift(Q);k?(I.addNgram(C,P),F.addNgram(R,P)):(F.addNgram(C,P),I.addNgram(R,P))}e.setupFromTries(F,I);return e};e.filteredIDs=function(){return K};e.search=function(B){E.search(B);D.search(B);B=d3.select(f[0]);var O=B.selectAll("text.foundText");if(O.empty())return 0;O=O[0].length;null!=B.selectAll("text.rtNdText.foundText").node()&&O--;return O};e.clearSearch=function(){E.clearSearch();
...
!1));G&&M.update(A)}function I(A){return k?-A:A}var E=u,D=q;f=f-20-20;g=g-20-20;var K=0,N;l||(l=doubletree.sortByStrFld("token"));K=0;var B=d3.tree().size([g,f]).nodeSize([40,40]),O=d3.linkHorizontal().x(function(A){return I(A.y)}).y(function(A){return A.x});e=e.append("g").attr("transform","translate(20,20)");this.readJSONTree=function(A){N=d3.hierarchy(A);N.x0=0;N.y0=0;N.children.forEach(z);this.update(N)};this.update=function(A){var G=B(N).descendants();G.forEach(function(C){var R=C.textSize;if(void 0==
R){R=0;for(var S=C.parent;null!=S;){var T=Ext.draw.TextMeasurer.measureText(S.data.name,"arial").width;R+=T;S=S.parent}C.textSize=R}C.y=25*C.depth+R});var J=e.selectAll("g.node_"+k).data(G,function(C){return C.id||(C.id=++K)}),P=J.enter().append("g").attr("class","node node_"+k).attr("cursor","pointer").attr("transform",function(C){return"translate("+I(A.y0)+","+A.x0+")"}).on("click",H);P.append("title").text(function(C){return doubletree.infoToText(C.data.info)});P.append("circle").attr("r",1E-6).style("fill",
...
this.nodes[E]instanceof f){var D=this.nodes[E];D.info.count+=H;D.info.ids[L]=!0;for(var K in I)E=I[K],-1==D.info[m[K]].indexOf(E)&&D.info[m[K]].push(E)}else D=new f(F,L,H),this.nodes[E]=D;F!=h&&D.addNgram(z,L,H)};this.getUniqRoot=function(){if(this.item==k){var z=Object.keys(this.nodes);if(1==z.length)return this.nodes[z[0]]}return this};this.toTree=function(z){function L(F,I,E){var D={children:[]};D.name=E.item;D.info={};for(var K in E.info)if("Object"===typeof E.info[K]){D.info[K]={};for(var N in E.info[K])D.info[K][N]=
this.info[K][N]}else D.info[K]=E.info[K];D.pruned={};for(var B in E.nodes)K=E.nodes[B],N=F[I],!N||N&&N(K.info)?(D.children.push(L(F,I+1,K)),K.pruned!={}&&g(D.pruned,K.pruned)):g(D.pruned,K.info.ids);D.info.continuations=D.children.length;0==D.children.length?(D.children=null,D.maxChildren=0,D.maxLen=D.name?D.name.length:0,D.minCount=D.info.count):(F=d3.max(D.children.map(function(O){return O.maxChildren})),D.maxChildren=Math.max(D.children.length,F),F=d3.max(D.children.map(function(O){return O.maxLen})),
...
collapsible:!0,collapsed:!0,defaultType:"textfield",items:[{xtype:"container",html:"\x3cp\x3e\x3ci\x3e"+(new Ext.Template(a.localize("tableOptionsText"))).applyTemplate([a.getBaseUrl()+"docs/tutorial-corpuscreator.html#tables"])+"\x3c/i\x3e\x3c/p\x3e",width:375},{xtype:"combo",fieldLabel:a.localize("tableDocuments"),name:"tableDocuments",queryMode:"local",store:[["",a.localize("tableDocumentsTable")],["rows",a.localize("tableDocumentsRows")],["columns",a.localize("tableDocumentsColumns")]],forceSelection:!0,
...
//# sourceMappingURL=voyant.ec9cace6f97220e04b15c428a172c5c8.min.js.map