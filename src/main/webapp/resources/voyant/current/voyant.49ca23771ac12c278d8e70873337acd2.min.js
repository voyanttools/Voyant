...
{query:a.term,start:a.start,limit:b}),callback:function(d,e,f){f&&(this.updateDataForNode(a.id,{start:a.start+b}),this.loadFromCorpusCollocateRecords(d,a.id))},scope:this})}});Ext.define("Voyant.panel.Constellations",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.constellations",statics:{i18n:{title:"Constellations",cutoff:"Similarity Threshold",numTerms:"Terms"},api:{docId:void 0,limit:50,dimensions:3,stopList:"auto"},glyph:"xf1e0@FontAwesome"},config:{caStore:void 0,allNodeData:void 0,allEdgeData:void 0,cutoff:50,vis:void 0,simulation:void 0,physics:{nodeGravity:-500,springLength:50},zoom:void 0,zoomExtent:[.25,8],options:[{xtype:"stoplistoption"}]},
constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){this.setCaStore(Ext.create("Voyant.data.store.CAAnalysis",{listeners:{load:this.handleData,scope:this}}));Ext.apply(this,{title:this.localize("title"),dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",items:[{xtype:"corpusdocumentselector"},{xtype:"slider",fieldLabel:this.localize("numTerms"),labelAlign:"right",labelWidth:50,width:200,
minValue:10,maxValue:250,increment:5,listeners:{render:function(a){a.setValue(this.getApiParam("limit"))},changecomplete:function(a,b){this.setApiParam("limit",b);this.loadFromApis()},scope:this}},{xtype:"slider",fieldLabel:this.localize("cutoff"),labelAlign:"right",labelWidth:150,width:300,minValue:0,maxValue:150,increment:1,listeners:{render:function(a){a.setValue(this.getCutoff())},changecomplete:function(a,b){this.setCutoff(b);this.updateGraph()},scope:this}}]}],listeners:{scope:this}});this.on("boxready",
...
this.getLayout().getRenderTarget();c.update("");var d=c.getWidth(),e=c.getHeight(),f=d3.select(c.dom).append("svg").attr("width",d).attr("height",e),g=f.append("g");g.append("g").attr("class","edges");g.append("g").attr("class","nodes");g.append("g").attr("class","labels");this.setVis(g);c=d3.zoom().scaleExtent(this.getZoomExtent()).on("zoom",function(h){g.attr("transform",h.transform)});this.setZoom(c);f.call(c);c=this.getPhysics();this.setSimulation(d3.forceSimulation(a).force("x",d3.forceX(d/2)).force("y",
d3.forceY(e/2)).force("link",d3.forceLink(b).id(function(h){return h.id}).distance(c.springLength)).force("charge",d3.forceManyBody().strength(c.nodeGravity)).force("center",d3.forceCenter(d/2,e/2)).on("tick",function(){f.select(".edges").selectAll("line").attr("x1",function(h){return h.source.x}).attr("y1",function(h){return h.source.y}).attr("x2",function(h){return h.target.x}).attr("y2",function(h){return h.target.y});f.select(".nodes").selectAll("circle").attr("cx",function(h){return h.x}).attr("cy",
function(h){return h.y});f.select(".labels").selectAll("text").attr("x",function(h){return h.x+15}).attr("y",function(h){return h.y+5});.075>this.getSimulation().alpha()&&this.getSimulation().alpha(-1)}.bind(this)).on("end",function(){Ext.Function.defer(this.zoomToFit,100,this)}.bind(this)));this.getSimulation().stop()},updateGraph:function(){var a=this.getCutoff()/1E3;a=$jscomp.makeIterator(this.doFilter(this.getAllNodeData(),this.getAllEdgeData(),a,""));nodes=a.next().value;edges=a.next().value;
this.getVis().select(".labels").selectAll("text").data(nodes).join("text").classed("hidden",!1).classed("label",!0).style("cursor","pointer").attr("font-size","16px").attr("font-family",'"Palatino Linotype", "Book Antiqua", Palatino, serif').text(function(b){return b.id}).on("click",this.handleNodeClick.bind(this)).call(d3.drag().on("start",this.handleDragStart.bind(this)).on("drag",this.handleDrag.bind(this)).on("end",this.handleDragEnd.bind(this)));this.getVis().select(".nodes").selectAll("circle").data(nodes,
function(b){return b.id}).join("circle").classed("node",!0).classed("selected",function(b){return!0===b.selected}).style("cursor","pointer").style("stroke","#6baed6").style("stroke-width",1).style("fill","#c6dbef").attr("r",function(b){return!0===b.selected?30:10}).on("click",this.handleNodeClick.bind(this)).call(d3.drag().on("start",this.handleDragStart.bind(this)).on("drag",this.handleDrag.bind(this)).on("end",this.handleDragEnd.bind(this)));this.getVis().select(".edges").selectAll("line").data(edges).join("line").classed("edge",
!0).style("stroke","#000").style("stroke-opacity",.1);this.getSimulation().nodes(nodes).force("link").links(edges);this.getSimulation().alpha(1).restart()},handleNodeClick:function(a,b){a.stopImmediatePropagation();a.preventDefault();this.dispatchEvent("termsClicked",this,[b.id])},handleDragStart:function(a){this.getSimulation().alpha(.3).restart();a.subject.fx=a.subject.x;a.subject.fy=a.subject.y},handleDrag:function(a){this.getSimulation().alpha(.3);a.subject.fx=a.x;a.subject.fy=a.y},handleDragEnd:function(a){a.subject.fx=
null;a.subject.fy=null},loadFromApis:function(){var a={};Ext.apply(a,this.getApiParams());this.getCaStore().load({params:a})},handleData:function(a){var b=a.getAt(0).getTokens().map(function(f){return f.getData()});b=b.filter(function(f){return"term"===f.category});a=b.map(function(f){return{id:f.term}});var c=[];b=$jscomp.makeIterator(this.combinations(b));for(var d=b.next();!d.done;d=b.next()){d=d.value;var e=this.distance(d[0].vector,d[1].vector);c.push({source:d[0].term,target:d[1].term,sim:e})}this.setAllNodeData(a);
this.setAllEdgeData(c);this.initGraph(a,c);this.updateGraph()},doFilter:function(a,b,c,d){if(void 0===a||void 0===b)return[[],[]];var e=new Set;b=b.filter(function(f){if(f.sim<=c)return e.add(f.source.id),e.add(f.target.id),!0});a=a.filter(function(f){f.selected=!1;return f.id===d?f.selected=!0:e.has(f.id)});0===a.length&&console.warn("no nodes!");console.log("num edges",b.length);return[a,b]},distance:function(a,b){return Math.sqrt(a.reduce(function(c,d,e){return c+Math.pow(d-b[e],2)},0))},combinations:function $jscomp$generator$function$873951774$46(a){var c,
d,e,f,g,h,k,l;return $jscomp.generator.createGenerator($jscomp$generator$function$873951774$46,function(m){switch(m.nextAddress){case 1:c=$jscomp.makeIterator(a.entries()),d=c.next();case 2:if(d.done){m.jumpTo(0);break}e=d.value;f=e[0];g=e[1];h=$jscomp.makeIterator(a.slice(f+1));k=h.next();case 5:if(k.done){d=c.next();m.jumpTo(2);break}l=k.value;return m.yield([g,l],6);case 6:k=h.next(),m.jumpTo(5)}})},zoomToFit:function(a,b){var c=this.getVis().node().getBBox(),d=c.width,e=c.height,f=c.x+d/2,g=c.y+
e/2;c=this.getVis().node().parentElement;var h=c.getBoundingClientRect(),k=h.width;h=h.height;a=(a||.8)/Math.max(d/k,e/h);f=[k/2-a*f,h/2-a*g];1>d||d3.select(c).transition().duration(b||500).call(this.getZoom().transform,d3.zoomIdentity.translate(f[0],f[1]).scale(a))}});Ext.define("Voyant.panel.Contexts",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],requires:["Voyant.data.store.Contexts"],alias:"widget.contexts",isConsumptive:!0,statics:{i18n:{},api:{query:void 0,docId:void 0,docIndex:void 0,stopList:"auto",context:5,expand:50,columns:void 0,sort:void 0,dir:void 0,termColors:"categories"},glyph:"xf0ce@FontAwesome"},config:{options:[{xtype:"stoplistoption"},{xtype:"categoriesoption"},{xtype:"termcolorsoption"}]},constructor:function(){this.mixins["Voyant.util.Api"].constructor.apply(this,
...
//# sourceMappingURL=voyant.49ca23771ac12c278d8e70873337acd2.min.js.map