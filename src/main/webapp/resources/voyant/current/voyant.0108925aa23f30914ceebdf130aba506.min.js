...
k)/100+"%\n";var l=0,m=Number.MAX_VALUE,n=0,r=Number.MAX_VALUE;this.getTermStore().removeAll();var w=[],p=[];c.getTokens().forEach(function(q){var u=q.get("rawFreq"),x=q.get("category");void 0===x&&(x="term",q.set("category","term"));var z="term"===x;z&&(u>l&&(l=u),u<m&&(m=u));this.getTermStore().findExact("term",-1===q.get("term"))&&this.getTermStore().addSorted(q);if(3===d){var G=q.get("vector")[2];void 0!==G&&(G<r&&(r=G),G>n&&(n=G))}u={x:q.get("vector")[0],y:q.get("vector")[1]||0,z:q.get("vector")[2]||
0,term:q.get("term"),rawFreq:u,relativeFreq:q.get("relativeFreq"),cluster:q.get("cluster"),category:x,disabled:!1};z?w.push(u):("bin"===q.get("category")?u.term=u.title="Bin "+q.get("docIndex"):(u.docIndex=q.get("docIndex"),q=this.getCorpus().getDocument(u.docIndex),null!==q&&(u.term=q.getShortTitle(),u.title=q.getTitle())),p.push(u))},this);this.updateLimit();c=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster category docIndex disabled".split(" "),data:w});g=Ext.create("Ext.data.JsonStore",
{fields:"term x y z rawFreq relativeFreq cluster category docIndex disabled".split(" "),data:p});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"],label:{rotate:{degrees:-30}}},{type:"numeric",position:"left",fields:["y"]}],sprites:[{type:"text",text:a,x:70,y:70}],innerPadding:{top:25,right:25,bottom:25,left:25},series:[{type:"customScatter",xField:"x",yField:"y",store:c,label:{font:"14px Helvetica",
field:"term",display:"over"},tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(q,u,x){q.setHtml(b.tokenFreqTipTemplate.apply([u.get("term"),u.get("rawFreq"),u.get("relativeFreq")]))}},marker:{type:"circle"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(q,u,x,z){q=x.store.getAt(z);if(null!==q){var G=q.get("cluster");-1===G&&(G=0);x=.65;z=1;!0===q.get("disabled")?z=x=.1:3===d&&q.get("z")&&(x=b.interpolate(q.get("z"),r,n,0,1));G=b.getApplication().getColor(G);u.fillStyle=
"rgba("+G.join(",")+","+x+")";u.strokeStyle="rgba("+G.join(",")+","+z+")";q=q.get("rawFreq");q=b.interpolate(q,m,l,2,20);u.radius=q}},scope:this},{type:"customScatter",xField:"x",yField:"y",store:g,label:{font:"14px Helvetica",field:"term",display:"over",color:this.getDefaultDocColor(!0)},tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(q,u,x){q.setHtml(b.docFreqTipTemplate.apply([u.get("title"),u.get("rawFreq")]))}},marker:{type:"diamond"},highlight:{fillStyle:"yellow",strokeStyle:"black"},
renderer:function(q,u,x,z){q=x.store.getAt(z);null!==q&&(x=q.get("cluster"),x=-1===x||"docSim"!==b.getApiParam("analysis")?b.getDefaultDocColor():b.getApplication().getColor(x),z=.65,3===d&&q.get("z")&&(z=b.interpolate(q.get("z"),r,n,0,1)),u.fillStyle="rgba("+x.join(",")+","+z+")",u.strokeStyle="rgba("+x.join(",")+",1)",u.radius=5)},scope:this}],listeners:{itemclick:function(q,u,x){q=u.record.data;"doc"===q.category?(q=this.getCorpus().getDocument(q.docIndex),this.getApplication().dispatchEvent("documentsClicked",
this,[q])):"term"===q.category&&(q=Ext.create("Voyant.data.model.CorpusTerm",q),this.getApplication().dispatchEvent("corpusTermsClicked",this,[q]))},render:function(q){q.body.on("contextmenu",function(u,x){u.preventDefault();u=u.getXY();var z=Ext.fly(x).getXY();x=u[0]-z[0];z=u[1]-z[1];var G=this.down("#chart").getItemForPoint(x,z);null!=G&&"term"===G.record.get("category")&&(x=this.down("#chart").getSeries(),x[0].disableToolTips(),x[1].disableToolTips(),z=G.record.get("term"),x=(new Ext.Template(this.localize("removeTerm"))).apply([z]),
this.getChartMenu().queryById("remove").setText(x),x=(new Ext.Template(this.localize("nearbyTerm"))).apply([z]),z=this.getChartMenu().queryById("nearby"),z.setText(x),z.setDisabled("tsne"===this.getApiParam("analysis")),this.getChartMenu().on("click",function(H,E){void 0!==E&&(H=G.record.get("term"),"nearby"===E.getItemId()?this.getNearbyForTerm(H):this.removeTerm(H))},this,{single:!0}),this.getChartMenu().showAt(u))},this)},scope:this}};a=Ext.create("Ext.chart.CartesianChart",a);this.queryById("chartParent").insert(0,
a);this.queryById("chartParent").unmask();this.doLabels();var v=this.getNewTerm();null!==v&&setTimeout(function(){this.selectTerm(v);this.setNewTerm(null)}.bind(this),0)},updateLimit:function(){var a=this.getCurrentTerms().length;this.setIgnoreTermsLimitChange(!0);this.queryById("limit").setValue(a);this.setIgnoreTermsLimitChange(!1);this.setApiParam("limit",a)},getDefaultDocColor:function(a){return this.getApplication().getColor(6,a)},doLabels:function(){var a=this.queryById("chart"),b=a.getSeries();
a=a.getSurface("chart").getItems()[0];var c=this.getApiParam("label");-1<c.indexOf("summary")?a.show():a.hide();-1<c.indexOf("terms")?b[0].getLabel().show():b[0].getLabel().hide();-1<c.indexOf("docs")?b[1].getLabel().show():b[1].getLabel().hide()},selectTerm:function(a,b){var c=this.down("#chart");if(null!==c)if(void 0===a)c.getSeries()[0].setHighlightItem(null),c.getSeries()[1].setHighlightItem(null);else{if(!0===b){var d=c.getSeries()[1];a=d.getStore().findExact("title",a)}else d=c.getSeries()[0],
a=d.getStore().findExact("term",a);if(-1!==a){var e=d.getStore().getAt(a),f=d.getSprites()[0];e={series:d,category:d.getItemInstancing()?"items":"markers",index:a,record:e,field:d.getYField(),sprite:f};d.setHighlightItem(e);b?c.getSeries()[0].setHighlightItem(null):c.getSeries()[1].setHighlightItem(null);b=this.getPointFromIndex(d,a);this.setHighlightData({x:b[0],y:b[1],r:50});null==this.getHighlightTask()&&this.setHighlightTask(Ext.TaskManager.newTask({run:this.doHighlight,scope:this,interval:25,
repeat:this.getHighlightData().r}));this.getHighlightTask().restart()}}},getPointFromIndex:function(a,b){a=a.getSprites()[0];return null!==a.surfaceMatrix?a.attr.matrix.clone().prependMatrix(a.surfaceMatrix).transformPoint([a.attr.dataX[b],a.attr.dataY[b]]):[0,0]},doHighlight:function(){var a=this.down("#chart");if(0<this.getHighlightData().r){for(var b=a.getSurface(),c=null,d=b.getItems(),e=0;e<d.length;e++){var f=d[e];if("customHighlight"==f.id){c=f;break}}null==c?b.add({id:"customHighlight",type:"circle",
strokeStyle:"red",fillStyle:"none",radius:this.getHighlightData().r,x:this.getHighlightData().x,y:this.getHighlightData().y}):(c.setAttributes({x:this.getHighlightData().x,y:this.getHighlightData().y,radius:this.getHighlightData().r}),this.getHighlightData().r-=1.5,0>=this.getHighlightData().r&&(this.getHighlightData().r=0,b.remove(c,!0)));a.redraw()}},filterChart:function(a){Ext.isString(a)&&(a=[a]);for(var b=[],c=0;c<a.length;c++)b.push(new RegExp(a[c]));a=this.queryById("chart");c=a.getSeries()[0];
var d=c.getLabel();c.getStore().each(function(e){var f=!1;if(0==b.length)f=!0;else for(var g=0;g<b.length&&!(f=f||b[g].test(e.get("term")));g++);e.set("disabled",!f);e=e.store.indexOf(e);d.setAttributesFor(e,{hidden:!f})},this);a.redraw()},getCurrentTerms:function(){var a=[];this.getTermStore().each(function(b){"term"===b.get("category")&&a.push(b.get("term"))});return a},getNearbyForTerm:function(a){var b=Math.max(2E3,Math.round(this.getCorpus().getWordTokensCount()/100));this.setApiParams({limit:b,
target:a});this.loadFromApis();this.setApiParam("target",void 0)},removeTerm:function(a){var b=this.down("#chart").getSeries()[0],c=b.getStore().findExact("term",a);b.getStore().removeAt(c);c=this.getTermStore().findExact("term",a);this.getTermStore().removeAt(c);this.updateLimit()},loadFromApis:function(a){this.queryById("chartParent").mask(this.localize("analyzing"));var b={},c=this.getCurrentTerms();null!==this.getNewTerm()&&(c=c.concat(this.getNewTerm()),this.setApiParam("limit",c.length));0<
c.length&&(null!==this.getNewTerm()||a)&&(b.query=c.join(","));Ext.apply(b,this.getApiParams());null!=b.target&&(b.term=c);"pca"===b.analysis?this.getPcaStore().load({params:b}):"tsne"===b.analysis?this.getTsneStore().load({params:b}):"docSim"===b.analysis?this.getDocSimStore().load({params:b}):this.getCaStore().load({params:b})},interpolate:function(a,b,c,d,e){return d+(e-d)*Math.max(0,Math.min(1,(a-b)/(c-b)))}});
...
window.open();w.document.write(m);w.document.close();w.focus();r.up("window").close()});this.exportWin.addListener("show",function(){var r=URL.createObjectURL(n);this.exportWin.down("#download").getEl().set({download:l,href:r})},this,{single:!0});e.resolve(this.exportWin)}.bind(this),function(f){e.reject(f)});return e.promise}},constructor:function(a){Ext.apply(a,{text:this.localize("text")});this.callParent(arguments)},glyph:"xf08e@FontAwesome",handler:function(a){Voyant.notebook.editor.button.Export.getExportWindow(a).then(function(b){b.show()},
...
function(){b.reject()})):"text"===d&&(c=c.down("textfield").getValue().replaceAll('"','"').replaceAll("\t","\\t").replaceAll("\n","\\n"),this.setValue(c),""!==c?b.resolve(a+'\x3dnew Spyral.Table("'+c+'")'):b.reject("No text entered!"));return b.promise},getTSV:function(){var a=void 0;this.getTable()&&(a=this.getTable().toTsv());return a},getArray:function(){var a=void 0;this.getTable()&&(a=this.getTable().toArray(!0));return a},getInput:function(){return{table:this.getArray(),type:this.getType(),
value:this.getValue()}},getBlob:function(){var a=new Ext.Deferred;a.resolve(this.getArray());return a.promise}});Ext.define("Voyant.notebook.editor.DataWrapper",{extend:"Voyant.notebook.editor.RunnableEditorWrapper",requires:["Voyant.notebook.editor.CodeEditor","Voyant.notebook.editor.FileInput","Voyant.notebook.editor.CorpusInput"],alias:"widget.notebookdatawrapper",cls:"notebook-data-wrapper",statics:{i18n:{dataNameAlphanum:"The data name must contain alphanumeric characters only",dataNameAlphastart:"The data name must not begin with a number"}},config:{dataName:void 0},cachedInput:void 0,constructor:function(a){a.mode=
...
//# sourceMappingURL=voyant.0108925aa23f30914ceebdf130aba506.min.js.map