...
a.getStore().loadRawData(b);var c=!0,d=a.getRawValue();b.forEach(function(e){-1!==e.indexOf(d)&&(c=!1)});c&&a.setValue(void 0)},getCode:function(a){var b=new Ext.Deferred;if(this.getCorpusId()){var c=a+'\x3dnew Spyral.Corpus("'+this.getCorpusId()+'")';b.resolve(c)}else{c=this.down("#cards").getLayout().getActiveItem();var d=c.getItemId();this.setType(d);"variable"===d?(c=c.down("combo").getValue(),this.setValue(c),""!==c?b.resolve("Spyral.Corpus.load({input:"+c+"}).then(function(){"+a+"\x3darguments[0]})"):
b.reject("No variable specified!")):"file"===d?(d=c.getFile(),this.setValue(c.getFileName()),Spyral.Util.blobToDataUrl(d).then(function(e){b.resolve("Spyral.Corpus.load(Spyral.Util.dataUrlToBlob(`"+e+"`)).then(function(){"+a+"\x3darguments[0]})")},function(){b.reject()})):"text"===d?(c=c.down("textfield").getValue().replace('"','"'),this.setValue(c),""!==c?b.resolve('Spyral.Corpus.load({input:"'+c+'"}).then(function(){'+a+"\x3darguments[0]})"):b.reject("No text entered!")):"id"===d&&(c=c.down("textfield").getValue().replace('"',
'"'),this.setValue(c),""!==c?b.resolve('Spyral.Corpus.load({corpus:"'+c+'"}).then(function(){'+a+"\x3darguments[0]})"):b.reject("No ID entered!"))}return b.promise},getInput:function(){return{corpusId:this.getCorpusId(),type:this.getType(),value:this.getValue()}}});Ext.define("Voyant.notebook.editor.DataWrapper",{extend:"Voyant.notebook.editor.RunnableEditorWrapper",requires:["Voyant.notebook.editor.CodeEditor","Voyant.notebook.editor.FileInput","Voyant.notebook.editor.CorpusInput"],alias:"widget.notebookdatawrapper",cls:"notebook-data-wrapper",statics:{i18n:{dataNameAlphanum:"The data name must contain alphanumeric characters only",dataNameAlphastart:"The data name must not begin with a number"}},config:{dataName:void 0},constructor:function(a){a.mode=void 0!==
a.mode?a.mode:"json";var b="file"===a.mode||"corpus"===a.mode;this.editor=Ext.create("Voyant.notebook.editor.CodeEditor",{content:Ext.Array.from(a.input).join("\n"),docs:a.docs,mode:a.mode,hidden:b,parentWrapper:this});this.cachedInput="corpus"===a.mode?Ext.create("Voyant.notebook.editor.CorpusInput",a.input):Ext.create("Voyant.notebook.editor.FileInput",Ext.apply(b?a.input:{},{hidden:!b}));b=!a.output||-1!==a.output.indexOf("\x3eundefined\x3c");this.results=Ext.create("Voyant.notebook.editor.SandboxWrapper",
...
runUntil:function(a){var b=[];Ext.Array.each(this.query("notebookrunnableeditorwrapper"),function(c){b.push(c);if(a&&a===c)return!1},this);this._run(b)},runFrom:function(a){var b=[],c=!1;Ext.Array.each(this.query("notebookrunnableeditorwrapper"),function(d){a&&a===d&&(c=!0);c&&b.push(d)},this);this._run(b)},runAll:function(){var a=[];Ext.Array.each(this.query("notebookrunnableeditorwrapper"),function(b){a.push(b)},this);this._run(a)},_run:function(a,b){if(0<a.length){var c=a.shift(),d=this;c.run(b).then(function(e){e=
c.getVariables();void 0===b&&(b=[]);e.forEach(function(f){for(var g=0;g<b.length;g++)if(f.name===b[g].name){b.splice(g,1);break}});b=b.concat(e);Ext.defer(d._run,100,d,[a,b])},function(e){})}else this.updateTernServerVariables(this.getNotebookVariables()),this.fireEvent("notebookRun",this)},notebookInitialized:function(){},updateTernServerVariables:function(a,b){Voyant.notebook.editor.CodeEditor.ternServer&&(b&&b.forEach(function(c){Voyant.notebook.editor.CodeEditor.ternServer.server.delFile(c.name)}),
