...
function(){this.loadFromString(c)})},fileSaving:function(){this.mask(this.localize("saving"))},fileSaved:function(c,d,e){this.unmask();null!==d?((c=this.getNotebookId())&&d==c||this.setNotebookId(d),this.toastInfo({html:this.localize("saved"),anchor:"tr"}),this.setIsEdited(!1)):this.showError("There was an error saving your notebook:\n"+e)},saveCancelled:function(){},close:function(){},scope:this}});this.githubDialogs=new Voyant.notebook.github.GitHubDialogs({parent:this,listeners:{fileLoaded:function(c,
d){c=d.owner;var e=d.repo,f=d.path;d=d.file;this.githubDialogs.close();this.loadFromString(d);d=encodeURIComponent(c+"/"+e+"/"+f);-1===location.search.indexOf(d)&&(c=this.getBaseUrl()+"spyral/?githubId\x3d"+d,window.history.pushState({url:c},"Spyral Notebook: "+d,c))},fileSaving:function(){this.mask(this.localize("saving"))},fileSaved:function(c,d){c=d.owner;var e=d.repo;d=d.path;this.githubDialogs.close();this.unmask();this.toastInfo({html:this.localize("saved"),anchor:"tr"});this.setIsEdited(!1);
d=encodeURIComponent(c+"/"+e+"/"+d);-1===location.search.indexOf(d)&&(c=this.getBaseUrl()+"spyral/?githubId\x3d"+d,window.history.pushState({url:c},"Spyral Notebook: "+d,c))},saveCancelled:function(c){this.unmask()},scope:this}});this.catalogueWindow=new Voyant.notebook.Catalogue({listeners:{notebookSelected:this.handleNotebookSelected,scope:this}})},init:function(){window.onbeforeunload=function(){if(this.getIsEdited())return""}.bind(this);Ext.Promise.all([Ext.Ajax.request({url:this.getApplication().getBaseUrlFull()+
"resources/spyral/docs/spyral.json"}),Ext.Ajax.request({url:this.getApplication().getBaseUrlFull()+"resources/spyral/docs/ecmascript.json"}),Ext.Ajax.request({url:this.getApplication().getBaseUrlFull()+"resources/spyral/docs/browser.json"})]).then(function(a){this.spyralTernDocs=Ext.JSON.decode(a[0].responseText);this.ecmaTernDocs=Ext.JSON.decode(a[1].responseText);this.browserTernDocs=Ext.JSON.decode(a[2].responseText);this.loadFromQueryParams()}.bind(this),function(){this.loadFromQueryParams()}.bind(this));
this.retrieveAccountInfo()},reset:function(){this.getScrollable().scrollTo(0,0);this.getScrollable().trackingScrollTop=0;this.getScrollable().trackingScrollLeft=0;this.setMetadata(new Spyral.Metadata);this.voyantStorageDialogs.reset();this.getComponent("cells").removeAll()},showSaveDialog:function(a){this.getMetadata().setDateNow("modified");this.getMetadata().set({userId:this.accountInfo.id});this.updateCachedOutput().then(function(b){},function(b){console.warn("Error updating cached results")}).finally(function(){var b=
this.generateExportJson(),c=this.generateExportMetadata(this.getMetadata()),d=this.getStorageSolution();a||"voyant"!==d||void 0===this.getNotebookId()?"github"===d?this.githubDialogs.showSave(b):this.voyantStorageDialogs.showSave(b,c,a?void 0:this.getNotebookName()):this.voyantStorageDialogs.doSave({notebookName:this.getNotebookName(),data:b,metadata:c})}.bind(this))},updateCachedOutput:function(){return Ext.Promise.all(this.query("notebookrunnableeditorwrapper").map(function(a){return a.results.updateCachedOutput()}))},
exportToolClick:function(a){a.mask(a.localize("preparingExport"));a.updateCachedOutput().finally(function(){a.unmask();a.mixins["Voyant.util.Toolable"].exportToolClick.call(this,a)})},getExportUrl:function(a){return location.href},exportBiblio:function(){var a=new Date,b=this.getExportUrl(),c=this.getMetadata(),d=c.author;c=c.title;Ext.Msg.show({title:this.localize("exportBiblioTitle"),message:'\x3cfieldset\x3e\x3clegend\x3eMLA\x3c/legend\x3e\x3cdiv class\x3d"x-selectable"\x3e'+d+'. "'+c+'." \x3ci\x3eSpyral - Voyant Tools\x3c/i\x3e, Development led by St\u00e9fan Sinclair and Geoffrey Rockwell. '+
