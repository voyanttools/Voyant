...
this,b[0])},query:function(a,b){this.fireEvent("termsClicked",this,b)}},loadCorpusTerms:function(a){if(2>this.getCorpus().getDocumentsCount()||"document"==this.getApiParam("mode")||0<Array.from(this.getApiParam("docIndex")||"").length)return this.loadDocumentTerms();if(this.getApiParam("query")){a=a||{};var b=this.getApiParam("withDistributions");Ext.applyIf(a,{bins:this.getCorpus().getDocumentsCount(),limit:100,stopList:""});var c=[],d=[];this.getCorpus().each(function(e){c.push(e.getTinyTitle());
d.push(e.getTitle())});Ext.Array.unique(c).length<c.length&&(c=c.map(function(e,f){return f+1+")"+e}));Ext.applyIf(a,this.getApiParams());this.getCorpus().getCorpusTerms().load({params:a,callback:function(e,f,g){var h=[],k=[],l=this.getApiParam("chartType");e.forEach(function(m,n){var u=m.get("term"),w=this.getApplication().getColorForTerm(u,!0);m.get("distributions").forEach(function(p,q){h[q]||(h[q]={index:c[q],docTitle:d[q]});h[q]["_"+n]="relative"==b?p.toFixed(7):p;h[q]["term"+n]=u},this);"bar"!=
l&&("barline"==l?["bar","line"]:[l]).forEach(function(p){k.push({type:"stacked"==p?"bar":p,title:u,xField:"index",yField:"_"+n,term:u,colors:[w],label:"barline"==l&&"bar"==p?{display:"none"}:{field:"term"+n}})},this)},this);f=e.map(function(m){return m.getTerm()});g=f.map(function(m){return this.getApplication().getColorForTerm(m,!0)},this);"bar"==l&&k.push({type:"bar",title:f,colors:g,xField:"index",yField:0<h.length?Object.keys(h[0]).filter(function(m){return"_"==m.charAt(0)}):void 0,label:{field:e.map(function(m,
n){return"term"+n})}});e=Ext.create("Ext.data.JsonStore",{fields:0<h.length?Object.keys(h[0]):void 0,data:h});this.buildChart({store:e,series:k,axes:[{type:"numeric",position:"left",increment:1,title:{text:this.localize(this.getApiParam("withDistributions")+"Title")}},{type:"category",position:"bottom",title:{text:this.localize("corpusTitle")}}]})},scope:this})}else this.getCorpus().getCorpusTerms().load({params:{limit:this.getApiParam("limit"),stopList:this.getApiParam("stopList")},callback:function(e,
f,g){0==e.length?f&&f.error?this.showError(this.localize("noResults")+"\x3cp style\x3d'color: red'\x3e"+f.error+"\x3c/p\x3e"):this.showError(this.localize("noResults")):(this.setApiParam("query",e.map(function(h){return h.getTerm()})),this.loadCorpusTerms())},scope:this})},loadDocumentTerms:function(a){if(this.getApiParam("query")){this.setApiParam("mode","document");a=a||{};var b=this.getApiParam("withDistributions");Ext.applyIf(a,{limit:0,sort:"termasc",stopList:void 0});var c=1==this.getCorpus().getDocumentsCount()?
this.getCorpus().getDocument(0):this.getCorpus().getDocument(this.getApiParam("docIndex"));Ext.applyIf(a,this.getApiParams());this.getCorpus().getDocumentTerms().load({params:a,callback:function(d,e,f){var g=[],h=[],k=this.getApiParam("chartType");c||d.sort(function(m,n){return m.getTerm()==n.getTerm()?m.getDocIndex()-n.getDocIndex():m.getTerm().localeCompare(n.getTerm())});d.forEach(function(m,n){var u=m.get("term"),w=m.get("docIndex"),p=c?this.getApplication().getColorForTerm(u,!0):this.getApplication().getColor(w,
!0);m.get("distributions").forEach(function(q,r){g[r]||(g[r]={index:r+1});g[r]["_"+n+"_"+w]="relative"==b?q.toFixed(7):q;g[r]["term"+n]=u},this);"bar"!=k&&("barline"==k?["bar","line"]:[k]).forEach(function(q){h.push({type:"stacked"==q?"bar":q,title:c?u:w+1+") "+u,xField:"index",yField:"_"+n+"_"+w,term:u,colors:[p],label:"barline"==k&&"bar"==q?{display:"none"}:{field:"term"+n}})},this)},this);if("bar"==k){var l=Ext.Array.unique(d.map(function(m){return m.getTerm()})).length;e=d.map(function(m){return 1+
m.get("docIndex")+") "+m.getTerm()});d=d.map(function(m){return l?this.getApplication().getColor(m.get("docIndex"),!0):this.getApplication().getColorForTerm(m.getTerm(),!0)},this);h.push({type:"bar",title:0<e.length?e:this.localize("noResults"),colors:d,xField:"index",yField:0<g.length?Object.keys(g[0]).filter(function(m){return"_"==m.charAt(0)}):void 0,label:{field:e}})}d=Ext.create("Ext.data.JsonStore",{fields:0<g.length?Object.keys(g[0]):void 0,data:g});this.buildChart({store:d,series:h,axes:[{type:"numeric",
position:"left",title:{text:this.localize(this.getApiParam("withDistributions")+"Title")}},{type:"category",position:"bottom",title:{text:this.localize("segmentsTitle")+(c?" ("+c.getTitle()+")":"")}}]})},scope:this})}else this.getCorpus().getCorpusTerms().load({params:{limit:this.getApiParam("limit"),stopList:this.getApiParam("stopList")},callback:function(d,e,f){this.setApiParam("query",d.map(function(g){return g.getTerm()}));this.loadDocumentTerms()},scope:this})},getItemToolTip:function(a,b,c){var d=
c.field.split("_"),e=2==d.length?c.index:d[2];d=parseInt(d[1]);var f=c.series.getTitle();f=Ext.isArray(f)?f[d]:f;var g=c.series.getColors();b="\x3cspan class\x3d'x-legend-item-marker' style\x3d'background:"+(1==g.length?g[0]:g[d])+"; left: 2px;'\x3e\x3c/span\x3e \x3cspan style\x3d'padding-left: 1.2em; font-weight: bold;'\x3e"+f+"\x3c/span\x3e: "+b.get(c.field)+"\x3cbr/\x3e\x3ci\x3e"+this.getCorpus().getDocument(e).getShortTitle()+"\x3c/i\x3e";b="corpus"==this.getApiParam("mode")?b+("\x3cdiv style\x3d'font-size: smaller'\x3e"+
this.localize("dblClickItem")):b+("\x3cbr/\x3e"+this.localize("segment")+" "+(c.index+1));a.setHtml(b)},buildChart:function(a){var b=this.getApiParam("chartType"),c=!1;if(!0===this.getApiParam("labels")||"true"==this.getApiParam("labels"))c=!0;Ext.applyIf(a,{cls:this.getApiParam("mode")});a.series.forEach(function(e){Ext.applyIf(e,{stacked:"bar"==e.type?!1:!0,showInLegend:"barline"==b&&"line"==e.type?!1:!0,smooth:!0,showMarkers:"bar"==e.type?!1:!0,marker:"barline"==b&&"line"==e.type?null:{type:"circle",
radius:2},style:{lineWidth:1,fillOpacity:"barline"==b&&"bar"==e.type?.01:1,strokeOpacity:"barline"==b&&"bar"==e.type?.1:1},highlight:!0,highlightCfg:{scaling:"bar"==e.type?1.1:2},label:{},tooltip:{trackMouse:!0,renderer:this.getItemToolTip,scope:this},listeners:{itemclick:function(f,g,h,k){this.clickTimer&&clearTimeout(this.clickTimer);if(!this.blockClick)if(this.blockClick=!0,Ext.defer(function(){this.blockClick=!1},1E3,this),"document"==this.getApiParam("mode"))f=g.field.split("_"),f=parseInt(f[2]),
h=this.getCorpus().getDocument(f).get("tokensCount-lexical"),h=parseInt(g.index*h/parseInt(this.getApiParam("bins"))),this.dispatchEvent("documentIndexTermsClicked",this,[{term:g.series.term,docIndex:f,position:h}]);else{this.clickTimer&&clearTimeout(this.clickTimer);var l=this;this.clickTimer=setTimeout(function(){l.dispatchEvent("documentIndexTermsClicked",l,[{term:g.series.term,docIndex:g.index}])},300)}},itemdblclick:function(f,g,h,k){this.clickTimer&&clearTimeout(this.clickTimer);this.blockClick=
...
this.loadStopwords()},parseLine:function(a){var b=this.getStopwords(),c=this.getVocabularyCounts(),d=this.getTokensPerTopic(),e=this.getVocabularySize();c=this.getVocabularyCounts();var f=this.getDocuments(),g=parseInt(this.getApiParam("numTopics")),h=this.getWordTopicCounts();if(""!=a){var k=f.length,l="",m=a.split("\t");a=m[0];3==m.length&&(k=m[0],l=m[1],a=m[2]);var n=[];m=a.toLowerCase().split(" ");if(null!=m){var u=this.zeros(g);m.forEach(function(w){if(""!==w){var p=Math.floor(Math.random()*
g);2>=w.length&&(b[w]=1);var q=b[w];q?c[w]=c[w]?c[w]+1:1:(d[p]++,h[w]||(h[w]={},e++,c[w]=0),h[w][p]||(h[w][p]=0),h[w][p]+=1,c[w]+=1,u[p]+=1);n.push({word:w,topic:p,isStopword:q})}});this.setVocabularySize(e);f.push({originalOrder:f.length,id:k,date:l,originalText:a,tokens:n,topicCounts:u})}}}});Ext.define("Voyant.panel.Via",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.via",statics:{i18n:{},api:{stopList:"auto",limit:100,docIndex:void 0},glyph:"xf06e@FontAwesome"},config:{mode:void 0,options:[{xtype:"stoplistoption"},{xtype:"listeditor",name:"whiteList"},{xtype:"categoriesoption"}]},layout:"fit",constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(a){Ext.apply(this,{title:this.localize("title"),
