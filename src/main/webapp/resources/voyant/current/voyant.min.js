/* This file created by JSCacher. Last modified: Wed Mar 06 20:03:46 UTC 2024 */
...
!0);m.get("distributions").forEach(function(q,r){g[r]||(g[r]={index:r+1});g[r]["_"+n+"_"+w]="relative"==b?q.toFixed(7):q;g[r]["term"+n]=u},this);"bar"!=k&&("barline"==k?["bar","line"]:[k]).forEach(function(q){h.push({type:"stacked"==q?"bar":q,title:c?u:w+1+") "+u,xField:"index",yField:"_"+n+"_"+w,term:u,colors:[p],label:"barline"==k&&"bar"==q?{display:"none"}:{field:"term"+n}})},this)},this);if("bar"==k){var l=1===Ext.Array.unique(d.map(function(m){return m.getTerm()})).length;e=d.map(function(m){return 1+
...
this.localize("expandResults"),handler:function(){this._doExpandCollapse()},scope:this},{itemId:"exportButton",glyph:"xf08e@FontAwesome",handler:function(d){d=Voyant.notebook.editor.EditorWrapper.toolbarLeft.down("notebookwrapperexport");Voyant.notebook.editor.button.Export.getExportWindow(d).then(function(e){e.show()},function(e){Ext.Msg.show({title:"Export Error",msg:"There was an error exporting the cell contents.\x3cbr\x3e\x3cpre style\x3d'color: red'\x3e"+e+"\x3c/pre\x3e",buttons:Ext.MessageBox.OK,
icon:Ext.MessageBox.ERROR})})},scope:this},{glyph:"xf014@FontAwesome",tooltip:"Remove Results",handler:function(){this.clear();this.hide()},scope:this}]}]},{xtype:"component",itemId:"expandWidget",height:20,hidden:b?!0:!1,style:{textAlign:"center",fontSize:"26px",cursor:"pointer",borderTop:"1px solid #DDD",color:"#000"},html:"\x26#8943;",listeners:{afterrender:function(d){var e=this;d.getEl().on("click",function(){e._doExpandCollapse()})},scope:this}}]});this.callParent(arguments)},initComponent:function(){var a=
this._handleResults.bind(this);this.on("afterrender",function(b){b.getEl().on("mouseover",function(d,e){b.down("#buttons").setVisible(!0)});b.getEl().on("mouseout",function(d,e){b.down("#buttons").setVisible(!1)});window.addEventListener("message",a);var c=this;this.setInitIntervalID(setInterval(function(){c._sendMessage({type:"command",command:"init"})},100))},this);this.on("removed",function(){window.removeEventListener("message",a)},this);this.callParent(arguments)},clear:function(){!1===this.isVisible()&&
(console.log("clearing but not visible!",this),this.show());this.down("#warnings").hide().warnings=[];this._sendMessage({type:"command",command:"clear"})},resetResults:function(){this.setCachedResultsValue(void 0);this.setCachedResultsOutput("");this.setCachedResultsVariables([])},run:function(a,b){void 0===b&&(b=[]);this.down("#warnings").hide().warnings=[];this.show();this.setHasRunError(!1);this.getEl().removeCls(["error","success","warning"]);this.setRunPromise(new Ext.Deferred);var c=this.getRunPromise().promise;
c.then(function(d){d.warnings&&0<d.warnings.length?(this._handleWarnings(d.warnings),this.getEl().addCls("warning")):this.getEl().addCls("success")},function(){this.setHasRunError(!0);this.getEl().addCls("error")},void 0,this);this.mask("Running code...");this._sendMessage({type:"code",value:a,variables:b});return c},mask:function(a){var b=this;this.setMaskTimeoutId(setTimeout(function(){b.superclass.mask.call(b,a,"spyral-code-mask")},250))},unmask:function(){clearTimeout(this.getMaskTimeoutId());
this.superclass.unmask.call(this)},updateHtml:function(a){this._sendMessage({type:"command",command:"update",html:a})},updateValue:function(a,b){this._sendMessage({type:"command",command:"update",name:a,value:b})},getValue:function(){return this.getCachedResultsValue()},updateCachedOutput:function(){var a=this;return new Ext.Promise(function(b,c){a._sendMessage({type:"command",command:"getContents"});a.on("sandboxMessage",function(d){"getContents"===d.command?a.setCachedResultsOutput(d.value):console.warn("getOutput: received unexpected message",
d);b(a.getCachedResultsOutput())},a,{single:!0})})},getOutput:function(){return this.getCachedResultsOutput()},getVariables:function(){return this.getCachedResultsVariables()},applyCachedResultsVariables:function(a,b){var c=this.up("notebook");c&&c.updateTernServerVariables(a,b);return a},getResultsEl:function(){var a=this.down("#resultsFrame");return a?a:null},_sendMessage:function(a){this.down("#resultsFrame").getWin().postMessage(a,"*")},_handleResults:function(a){var b=this.down("#resultsFrame").getFrame();
if(a.source===b.contentWindow)if(b=a.data,b.type){switch(b.type){case "error":console.log("iframe error:",b);this.unmask();this.getRunPromise().reject(b);break;case "command":switch(b.command){case "init":!1===this.getIsInitialized()&&(this.setIsInitialized(!0),clearInterval(this.getInitIntervalID()),this.fireEvent("initialized",this));break;case "clear":this._setHeight(0)}break;case "result":console.log("iframe result:",b),this.unmask(),this.getRunPromise().resolve(b)}"getContents"!==b.command&&
"clear"!==b.command&&(b.value&&this.setCachedResultsValue(b.value),this.setCachedResultsVariables(b.variables));b.output&&this.setCachedResultsOutput(b.output);0<b.height&&this._setHeight(b.height);this.fireEvent("sandboxMessage",b)}else console.warn("unrecognized message!",a)},_handleWarnings:function(a){var b=[];a.forEach(function(c){switch(c.type){case "serialization":b.push(this.localize("serializationWarning").replace("{warningInfo}",c.warningInfo));break;case "loadVariable":b.push(this.localize("loadVariableWarning").replace("{warningInfo}",
c.warningInfo))}},this);a=this.down("#warnings");a.show();a.warnings="\x3cp\x3e"+b.join("\x3c/p\x3e\x3cp\x3e")+"\x3c/p\x3e"},_doExpandCollapse:function(){var a=this.down("#expandButton");this.getExpandResults()?(this.setExpandResults(!1),a.setTooltip(this.localize("expandResults")),a.setGlyph("xf065@FontAwesome")):(this.setExpandResults(!0),a.setTooltip(this.localize("collapseResults")),a.setGlyph("xf066@FontAwesome"));this._setHeight()},_setHeight:function(a){if(void 0===this.getCachedPaddingHeight()){var b=
window.getComputedStyle(this.getEl().dom);this.setCachedPaddingHeight(parseFloat(b.getPropertyValue("padding-top"))+parseFloat(b.getPropertyValue("padding-bottom"))+2)}void 0!==a?this.setCachedResultsHeight(a):(a=this.getCachedResultsHeight(),void 0===a&&(a=(b=this.getResultsEl())?b.getHeight():this.getEmptyResultsHeight(),this.setCachedResultsHeight(a)));a+=this.getCachedPaddingHeight();if(b=this.getResultsEl())b=this.down("#expandWidget"),this.getExpandResults()?(b.hide(),this.setHeight(Math.max(a,
this.getEmptyResultsHeight()))):(a=Math.min(Math.max(a,this.getEmptyResultsHeight()),this.getMinimumResultsHeight()),a<this.getMinimumResultsHeight()?b.hide():b.show(),this.setHeight(a))}});Ext.define("Voyant.notebook.editor.CodeEditor",{extend:"Ext.Component",alias:"widget.notebookcodeeditor",mixins:["Voyant.util.Localization"],cls:"notebook-code-editor",config:{mode:"javascript",content:"",docs:void 0,isChangeRegistered:!1,editor:void 0,editedTimeout:void 0,editorHeight:void 0,editorDocHeight:void 0,markers:[],parentWrapper:void 0,isCollapsed:!1},statics:{i18n:{},api:{content:void 0},ternServer:void 0},MIN_LINES:2,constructor:function(a){this.callParent(arguments)},listeners:{render:function(){var a=
...
a,c)}})}}]}}}).show();return!1}return!0},hasQueryToLoad:function(a){a||(a=Ext.Object.fromQueryString(document.location.search));return a.corpus||a.input||this.getCorpusId&&this.getCorpusId()},listeners:{loadedCorpus:function(a,b){this.setCorpus(b);this.on("unhandledEvent",function(c,d,e){c=this.getBaseUrl()+"?corpus\x3d"+b.getAliasOrId();var f=this.getModifiedApiParams()||{};delete f.view;if("termsClicked"==d)if(Ext.isArray(e)&&0<e.length&&"string"!==typeof e[0]&&"term"in e[0]&&"docIndex"in e[0]){var g=
{},h={};e.forEach(function(l){g[l.term]=!0;h[l.docIndex]=!0});f.query=Object.keys(g);f.docIndex=Object.keys(h)}else f.query=e;else if("documentsClicked"==d){var k=[];e.forEach&&e.forEach(function(l){k.push(l.getIndex())});f.docIndex=k}else if("corpusTermsClicked"==d)e.map&&(f.query=e.map(function(l){return l.getTerm()}));else if("documentTermsClicked"==d)e.map&&(f.query=e.map(function(l){return l.getTerm()}),f.docIndex=e.map(function(l){return l.getDocIndex()}));else{console&&console.warn("Unhandled event: "+
d,e);return}f.toolFlow&&(d=Ext.Array.from(f.toolFlow.split(",")),f.view=d.shift(),0<d.length?f.toolFlow=d.join(","):delete f.toolFlow);c+="\x26"+Ext.Object.toQueryString(f);this.openUrl(c)})}}});Ext.define("Voyant.panel.DocumentClusters",{extend:"Ext.panel.Panel",alias:"widget.documentclusters",title:"Document Clusters"});
