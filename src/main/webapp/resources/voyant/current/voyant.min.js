...
this.preparePath(a.word),d,e=d3.line().x(function(f){d=f.x;return b.xScale(f.x+b.callOffset())}).y(function(f){return f.y}).curve(d3.curveNatural);a=this.chart.select(".overlay").append("g").attr("class","frequency-line-"+a.selector).append("path").attr("d",e(c)).style("stroke",a.colour).style("stroke-width",2).style("fill","none");c=this.xScale(d)-this.xScale(d+this.callOffset());"left"!==this.transitionCall&&"right"!==this.transitionCall||a.transition().duration(b.getDuration()).ease(d3.easeLinear).attr("transform",
...
b,[h.data.term])}).on("mouseover",function(h,k){b.setCurrentNode(h);b.getVis().selectAll("circle").style("stroke-width",1).style("stroke","#111").style("fill",function(n){return e(n.data.fillValue)});d3.select(this).select("circle").style("fill","#89e1c2").style("stroke","#26926c").style("stroke-opacity",b.MAX_STROKE_OPACITY);k=b.getMode()===b.MODE_DISTINCT?b.localize("tfidf"):b.localize("inDocs");if(!b.getContextMenu().isVisible()){k="\x3cb\x3e"+h.data.term+"\x3c/b\x3e ("+h.data.rawFreq+")\x3cbr/\x3e"+
