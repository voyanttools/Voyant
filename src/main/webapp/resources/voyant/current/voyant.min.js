...
k.palette]),n.resolve())):n.resolve();n.promise.always(function(){var u=l.getCorpus();if(0<m.length){var w=l.getViewport().query("panel,chart");m.forEach(function(p){var q=p[0];p=p[1];l.setApiParam&&l.setApiParam(q,p);for(var r=0;r<w.length;r++)w[r].setApiParam&&w[r].setApiParam(q,p)});u?l.dispatchEvent("loadedCorpus",this,u):l.dispatchEvent("apiParamsUpdated",this,k)}u?this.fireEvent("loadedCorpus",this,u):this.fireEvent("apiParamsUpdated",this,k);h.up("window").close()}.bind(this))},scope:g}]},
...
this.getDocumentTermsStore().getProxy().setExtraParam("corpus",a.getId());this.setIsDetailedGraph(b.getTotalCount()<this.DETAILED_GRAPH_DOC_LIMIT);this.generateChart(a,this)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a}})},generateChart:function(a,b){function c(u,w){u=this.getParentPanel().getApplication().getColor(u);return"rgba("+u[0]+","+u[1]+","+u[2]+","+w+")"}function d(u){var w=u.index,p=u.fraction;u=u.relativeHeight;var q=c.call(this,w,.3);w=
c.call(this,w,1);b.add({xtype:"cartesian",plugins:{ptype:"chartitemevents"},flex:p,height:"100%",insetPadding:0,background:{type:"linear",degrees:90,stops:[{offset:0,color:q},{offset:u,color:q},{offset:u,color:"white"},{offset:1,color:"white"}]},axes:[{type:"numeric",position:"left",fields:"distribution",hidden:!0},{type:"category",position:"bottom",fields:"bin",hidden:!0}],series:[{type:"line",xField:"bin",yField:"distribution",style:{lineWidth:2,strokeStyle:w},tooltip:{corpus:a,trackMouse:!0,style:"background: #fff",
showDelay:0,dismissDelay:500,hideDelay:5,renderer:function(r,v,x){r.setHtml(a.getDocument(v.get("docIndex")).getTitle()+"\x3cbr\x3e"+v.get("term")+": "+v.get("distribution"))}}}],store:Ext.create("Ext.data.ArrayStore",{fields:["docId","docIndex","bin","distribution","term"],data:[]}),listeners:{itemclick:function(r,v,x){},scope:this}}).body.on("click",function(r,v){v=Ext.get(v);r=r.getX();var x=v.getBox();r=(r-x.x)/x.width;v=v.parent(".x-panel");x=v.parent();v=Ext.toArray(x.dom.children).indexOf(v.dom);
this.fireEvent("documentRelativePositionSelected",this,{docIndex:v,fraction:r})},this)}b.removeAll();for(var e=a.getDocuments(),f=a.getWordTokensCount(),g=[],h=Number.MAX_VALUE,k=-1,l=0;l<e.getCount();l++){var m=e.getAt(l),n=m.get("index");m=m.get("tokensCount-lexical");m<h&&(h=m);m>k&&(k=m);g.push({index:n,count:m,fraction:m/f})}if(this.getIsDetailedGraph())for(l=0;l<g.length;l++)m=g[l],m.relativeHeight=m.count==k?1:.25+(m.count-h)/(k-h)*.75,d.call(this,m);else b.add({xtype:"cartesian",plugins:{ptype:"chartitemevents"},
flex:1,height:"100%",insetPadding:0,axes:[{type:"numeric",position:"left",fields:"count",hidden:!0},{type:"category",position:"bottom",fields:"index",hidden:!0}],series:[{type:"bar",xField:"index",yField:"count",style:{minGapWidth:0,minBarWidth:1,lineWidth:0,strokeStyle:"none"},renderer:function(u,w,p,q){return{fillStyle:c.call(this,q,.3)}}.bind(this)}],store:Ext.create("Ext.data.JsonStore",{fields:[{name:"index",type:"int"},{name:"count",type:"int"},{name:"fraction",type:"float"}],data:g}),listeners:{itemclick:function(u,
w,p){u=Ext.get(p.getTarget());p=p.getX();u=u.getBox();var q=u.width/this.getCorpus().getDocuments().getCount();p=(p-u.x)%q/q;w=w.record.data;w=this.getDocumentsStore().getAt(w.index).getIndex();this.fireEvent("documentRelativePositionSelected",this,{docIndex:w,fraction:p})},scope:this}})},moveLocationMarker:function(a,b,c){var d=Ext.get(this.getLocationMarker()),e=d.getX();if(this.getIsDetailedGraph()){var f=this.query("cartesian")[a];f&&(e=f.getX()+f.getWidth()*b)}else f=this.down("cartesian"),e=
...
d=h-this.getApiParam("limit")/2;e=this.getCorpus().getDocument(g);this.setApiParams({skipToDocId:e.getId(),start:0>d?0:d});this.load(!0,{callback:function(){var k=this.body.dom.querySelector("#_"+g+"_"+h);k&&k.scrollIntoView();this.highlightKeywords(f.get("term"),!1)},scope:this})}},documentIndexTermsClicked:function(d,e){void 0!==e[0]&&(d=Ext.create("Voyant.data.model.Token",e[0]),this.fireEvent("termLocationClicked",this,[d]))},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&
0<a.length&&(this.getDocumentTermsStore().load({params:{query:a,categories:this.getApiParam("categories")}}),this.down("readergraph").loadQueryTerms(a))},highlightKeywords:function(a,b){Ext.isArray(a)||(a=[a]);this.getInnerContainer().first().select("span[class*\x3dkeyword]").removeCls("keyword");var c=[],d=new RegExp("^"+a[0]+"$","i");this.getInnerContainer().first().select("span.word").each(function(e,f,g){e.dom.firstChild&&e.dom.firstChild.nodeValue.match(d)&&(e.addCls("keyword"),c.push(e.dom))})},
nerSeviceHandler:function(a){a=a.itemId;var b=[],c=this.getLocationInfo();if(c)for(var d=c[0].docIndex;d<=c[1].docIndex;d++)b.push(d);else b.push(0);this.clearEntityHighlights();var e=this;new Voyant.data.util.DocumentEntities({annotator:a,includeEntities:!0,docIndex:b},function(f){f&&(e.clearEntityHighlights(),e.setDocumentEntitiesStore(f),e.highlightEntities())})},clearEntityHighlights:function(){this.getInnerContainer().first().select(".entity").each(function(a){a.removeCls("entity start middle end location person organization misc money time percent date duration set unknown");
a.dom.setAttribute("data-qtip",a.dom.getAttribute("data-qtip").replace(/<div class="entity">.*?<\/div>/g,""))})},highlightEntities:function(){var a=this.getInnerContainer().first(),b=this.getDocumentEntitiesStore(),c=this.localize("entityType");b.forEach(function(d){var e=d.positions;e?e.forEach(function(f){var g=2===f.length;if(g&&1<f[1]-f[0])for(var h=f[1],k=f[0]+1,l=1;k<h;)f.splice(l,0,k),k++,l++;h=0;for(k=f.length;h<k;h++)if(l=f[h],-1===l)console.warn("missing position for: "+d.term);else if(l=
a.selectNode("#_"+d.docIndex+"_"+l,!1)){var m="";g&&(m=0===h?"start ":h===k-1?"end ":"middle ");l.addCls("entity "+m+d.type);l.dom.setAttribute("data-qtip",l.dom.getAttribute("data-qtip")+'\x3cdiv class\x3d"entity"\x3e'+c+": "+d.type+"\x3c/div\x3e")}}):console.warn("no positions for: "+d.term)})},fetchPrevious:function(a){var b=this.getInnerContainer().first(),c=b.first(".word");if(null!=c&&!1===c.hasCls("loading"))for(;c;){if(c.hasCls("word")){b=Voyant.data.model.Token.getInfoFromElement(c);var d=
...
n){var u=m.get("term"),w=m.get("docIndex"),p=c?this.getApplication().getColorForTerm(u,!0):this.getApplication().getColor(w,!0);w=m.get("docIndex");m.get("distributions").forEach(function(q,r){g[r]||(g[r]={docIndex:w,index:r+1});g[r]["_"+n+"_"+w]="relative"==b?q.toFixed(7):q;g[r]["term"+n]=u},this);"bar"!=k&&("barline"==k?["bar","line"]:[k]).forEach(function(q){h.push({type:"stacked"==q?"bar":q,title:c?u:w+1+") "+u,xField:"index",yField:"_"+n+"_"+w,term:u,colors:[p],label:"barline"==k&&"bar"==q?{display:"none"}:
...
arguments);this.mixins["Voyant.util.Colors"].constructor.apply(this,arguments);this.setCategoriesManager(new Spyral.Categories);this.getCategoriesManager().addFeature("color");this.getCategoriesManager().addFeature("font",'"Palatino Linotype", "Book Antiqua", Palatino, serif');var b=this.getColor;this.getColor=function(d,e){return b.apply(this,[this.getApiParam("palette"),d,e])};var c=this.getColorForTerm;this.getColorForTerm=function(d,e){return c.apply(this,[this.getApiParam("palette"),d,e])};this.callParent(arguments)},
getBaseUrl:function(){var a=this.callParent();return 0==a.indexOf("//")?location.protocol+a:a},getBaseUrlFull:function(){return window.location.origin+this.getBaseUrl()},getRelativeUrl:function(){for(var a="",b=0,c=window.location.pathname.substring(this.getBaseUrl().length).split("/").length-1;b<c;b++)a+="../";return a},getTools:function(){return[{type:"maximize"},{type:"help"}]},launch:function(){Ext.tip.QuickTipManager.init();Ext.apply(Ext.tip.QuickTipManager.getQuickTip(),{showDelay:50});this.callParent(arguments)},
...
a.getError()+" \u2026 \x3ca href\x3d'#' onclick\x3d\"window.open('').document.write(unescape('\x3cpre\x3e"+escape(a.getDetails())+"\x3c/pre\x3e')); return false;\"\x3emore\x3c/a\x3e\x3c/p\x3e"};else{if(a instanceof Ext.data.operation.Operation)return this.showError(Ext.create("Voyant.util.ResponseError",a.getError()));if(Ext.isString(a))a={message:a};else if(Ext.isObject(a)&&(a.responseText||a.statusText))return this.showResponseError(a.statusText,a)}Ext.applyIf(a,{title:this.localize("error"),buttons:Ext.Msg.OK,
icon:Ext.MessageBox.ERROR,autoScroll:!0});Ext.Msg.show(a)},getToolConfigFromToolXtype:function(a){cls=Ext.ClassManager.getByAlias("widget."+a);return{xtype:a,title:this._localizeClass(cls,"title"),tooltip:{text:this._localizeClass(cls,"helpTip")},glyph:cls&&cls.glyph?cls.glyph:"xf12e@FontAwesome"}},openUrl:function(a){window.open(a)||Ext.Msg.show({title:"Popup Blocked",buttonText:{ok:"Close"},icon:Ext.MessageBox.INFO,message:"A popup window was blocked. \x3ca href\x3d'"+a+"' target\x3d'_blank' class\x3d'link'\x3eClick here\x3c/a\x3e to open the new window.",
