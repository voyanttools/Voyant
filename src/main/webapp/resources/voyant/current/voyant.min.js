...
message:a.localize("pleaseProvideText"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})}}]}],items:[{html:this.getInitialConfig().addTextLabel,hidden:void 0==this.getInitialConfig().addTextLabel},{height:100,xtype:"textareafield",itemId:"input",emptyText:this.localize("emptyInput")}]});a.on("boxready",function(b){var c=this.getApplication();c.getAllowInput&&"false"==c.getAllowInput()&&(b.getDockedItems().forEach(function(d){b.removeDocked(d)}),b.removeAll(),b.add({xtype:"container",html:"\x3cp\x3e"+b.localize("noAllowInputMessage")+
