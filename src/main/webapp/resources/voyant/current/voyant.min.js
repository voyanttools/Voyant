...
g+";";if(d.get("positions")){e=d.get("positions");var k=d.get("docIndex");e.forEach(function(m){(m=c.dom.querySelector("#_"+k+"_"+m))&&Ext.fly(m).addCls("keyword").dom.setAttribute("style",h)})}else{var l=new RegExp("^"+e+"$","i");c.select("span.word").each(function(m,n,u){m.dom.firstChild&&m.dom.firstChild.nodeValue.match(l)&&m.addCls("keyword").dom.setAttribute("style",h)})}},this))},nerServiceHandler:function(a){a=a.itemId;var b=[],c=this.getLocationInfo();if(c)for(var d=c[0].docIndex;d<=c[1].docIndex;d++)b.push(d);
...
[],d=[];this.getCorpus().each(function(e){c.push(e.getTinyTitle());d.push(e.getTitle())});Ext.Array.unique(c).length<c.length&&(c=c.map(function(e,f){return f+1+")"+e}));Ext.applyIf(a,this.getApiParams());this.getCorpus().getCorpusTerms().load({params:a,callback:function(e,f,g){var h=[],k=[],l=this.getApiParam("chartType");e.forEach(function(m,n){var u=m.get("term"),w=this.getApplication().getColorForTerm(u,!0);m.get("distributions").forEach(function(p,q){h[q]||(h[q]={index:c[q],docTitle:d[q]});h[q]["_"+
