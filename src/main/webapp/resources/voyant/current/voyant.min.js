...
type:this.getApiParam("type"),limit:this.getApiParam("limit"),minEdgeCount:this.getApiParam("minEdgeCount"),corpus:a},timeout:6E4,success:function(a){b.unmask();a=Ext.decode(a.responseText);0==a.entityCollocationsGraph.edges.length?(this.showError({msg:this.localize("noEntities")}),Ext.Msg.confirm(this.localize("error"),this.localize("noEntitiesForEdgeCount"),function(a){"yes"===a&&(a=Math.max(1,this.getApiParam("minEdgeCount")-1),this.queryById("minEdgeCount").setRawValue(a),this.setApiParam("minEdgeCount",
a),this.getEntities())},this)):(this.processEntities(a.entityCollocationsGraph),this.initGraph())},scope:this})},processEntities:function(a){var b=a.nodes;a=a.edges;var c=d3.extent(b,function(a){return a.rawFreq}),d=c[0];c=c[1];var e=d3.scaleLinear().domain([d,c]).range([10,24]);d=[];for(c=0;c<b.length;c++){var f=b[c];f.id=c;d.push({id:c,label:f.term,value:b[c].rawFreq,font:{size:e(f.rawFreq),color:this.categorizedNodeOptions[f.type].font.color},type:f.type,rawFreq:f.rawFreq,title:f.term+(f.rawFreq?
...
nodes:this.nodeOptions,edges:this.edgeOptions},c=new vis.Network(a.dom,{nodes:this.getNodesDataSet(),edges:this.getEdgesDataSet()},b);if(this.getIsNetworkBounded())c.on("beforeDrawing",function(a){var b=this.getLayout().getRenderTarget();a=b.getWidth();b=b.getHeight();var d=c.getPositions(),g;for(g in d){var h=c.canvasToDOM(d[g]);h=c.DOMtoCanvas({x:Math.max(0,Math.min(a,h.x)),y:Math.max(0,Math.min(b,h.y))});c.body.nodes[g].x=h.x;c.body.nodes[g].y=h.y}}.bind(this));c.on("selectNode",function(a){this.doNodeSelect(a.nodes[0])}.bind(this));
